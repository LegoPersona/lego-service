...
def combine_modules(persona: Persona) -> str:
	...
	for key, module in persona:
		if key != "skin_tone":
			...
			# Fall back to the primary color if no secondary was supplied, so a template's SECONDARY
			# tokens never survive un-substituted into the final LDR (which would break rendering).
